...
public class DiagramPartsExtractor {
	...
	private EventSequenceConditions eventSequenceConditions = null;
	...
	public EventSequenceConditions getEventSequenceConditions() {
		return eventSequenceConditions;
	}
			...
			if (diag.eContents().get(i) instanceof PictogramElement) {
				...
				for (int j = 0; j < objs.size(); j++) {
					if (obj instanceof CbCFormula && objs.get(j) instanceof CbCFormula) {
						...
						return;
					} else if (obj instanceof EventSequenceConditions
							&& objs.get(j) instanceof EventSequenceConditions) {
						objs.set(j, (EventSequenceConditions) obj);
						this.eventSequenceConditions = (EventSequenceConditions) obj;
						return;
					}
				}
			}
